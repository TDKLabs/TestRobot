...
class XmlLogger(ResultVisitor):
    ...
    def _get_writer(self, path, generator):
        ...
        writer.start('robot', {'generator': get_full_version(generator),
                               'generated': '%s %s'%(get_timestamp(),time.tzname[0])})
        return writer
    ...
    def _write_message(self, msg):
        attrs = {'timestamp': "%s %s"%(msg.timestamp if msg.timestamp else 'N/A',time.tzname[0]), 'level': msg.level}
        if msg.html:
            attrs['html'] = 'yes'
    ...
    def _write_status(self, item, extra_attrs=None):
        attrs = {'status': item.status, 'starttime': "%s %s"%(item.starttime if item.starttime else 'N/A',time.tzname[0]),
                 'endtime': "%s %s"%(item.endtime if item.endtime else 'N/A',time.tzname[0])}
